...
const BUILTIN_JOBS = {
  SMN: [
    { id: "smn-ruin", name: "毁荡", gcd: 2.5, type: "gcd", icon: "assets/icons/smn_ruin.svg" },
    { id: "smn-gemshine", name: "宝石耀", gcd: 2.5, type: "gcd", icon: "assets/icons/smn_gemshine.svg" },
    {
      id: "smn-energy-drain",
      name: "能量吸收",
      gcd: 1.0,
      type: "ability",
      icon: "assets/icons/smn_energy_drain.svg",
    },
    { id: "smn-fester", name: "溃烂爆发", gcd: 1.0, type: "ability", icon: "assets/icons/smn_fester.svg" },
  ],
};
...
function bindEvents() {
  ...
  els.skillForm.addEventListener("submit", async (event) => {
    ...
    currentSkills().push({ id: crypto.randomUUID(), name, gcd, type, icon: DEFAULT_ICON });
...
function ensureJobSlots() {
  ...
  if (!state.jobs.SMN) {
    state.jobs.SMN = BUILTIN_JOBS.SMN.map((skill) => ({ ...skill }));
  }
...
function createDefaultState() {
  return {
    currentJob: "SMN",
    jobs: { SMN: BUILTIN_JOBS.SMN.map((skill) => ({ ...skill })) },
...
async function parseXlsxEvents(file) {
  ...
  for (const rawRow of rows) {
    ...
    const row = rawRow.map((cell) => normalizeCell(cell));
    const maybePhase = row.find((cell) => /^P\d+$/i.test(cell));
    if (maybePhase) {
      currentPhase = maybePhase.toUpperCase();
    }

    const time = findRowTime(row);
    ...
    events.push({
      time,
      name: currentPhase ? `${currentPhase} ${name}` : name,
    });
  }
...
function findRowTime(row) {
  for (const cell of row) {
    const parsed = parseMixedTime(cell);
    if (parsed !== null) return parsed;
  }
  return null;
}

function parseMixedTime(value) {
  ...
  if (Number.isFinite(asNumber)) {
    if (asNumber >= 0 && asNumber < 1) return asNumber * 86400;
  ...
  const head = Number(timeMatch[1]);
  const mid = Number(timeMatch[2]);
  const tail = Number(timeMatch[3]);
  if ([head, mid, tail].some((n) => Number.isNaN(n))) return null;

  return head * 60 + mid + tail / 100;
}

function findRowName(row) {
  for (const cell of row) {
    if (!cell) continue;
    if (/^P\d+$/i.test(cell)) continue;
    if (parseMixedTime(cell) !== null) continue;
    if (!/[\u4e00-\u9fa5A-Za-z]/.test(cell)) continue;
    if (isMetaToken(cell)) continue;
    return cell;
  }
  return "";
}

function isMetaToken(text) {
...
function shouldSkipName(name) {
  ...
  return skipList.some((token) => name.includes(token));
}
...
function dedupeAndSortEvents(events) {
  ...
  for (const event of sorted) {
    const duplicated = out.some(
      (existing) => Math.abs(existing.time - event.time) < 0.02 && existing.name === event.name
    );
    if (!duplicated) out.push(event);
  }
